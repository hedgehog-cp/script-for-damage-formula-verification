namespace kcv {
  ...
  /**
   * 電探であるかを検証します.
   * @param { kcv.api_mst_slotitem } mst 装備マスタ
   * @returns { boolean } 電探であればtrueを返します.
   */
  export function is_radar(mst: kcv.api_mst_slotitem): boolean {
   ...
   */
  export function is_anti_air_radar(mst: kcv.api_mst_slotitem): boolean {
   ...
   */
  export function is_surface_radar(mst: kcv.api_mst_slotitem): boolean {
   ...
   */
  export function is_accuracy_radar(mst: kcv.api_mst_slotitem): boolean {
    return is_radar(mst) && mst.api_houm >= 8;
  }

  export function to_original_id(mst: kcv.api_mst_ship): number | undefined {
    // 線形探索したくない
    return kcv.api_mst_ship
